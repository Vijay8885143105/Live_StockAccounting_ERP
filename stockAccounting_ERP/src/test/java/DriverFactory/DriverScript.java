			...
			if(xl.getCellData(TestCases, i, 2).equalsIgnoreCase("Y"))
			{
				...
				for(int j=1;j<=xl.rowCount(TCModule);j++)
				{
					...
					try {
						...
						{
							FunctionLibrary.openApplcation(driver);
							logger.log(LogStatus.INFO, Description);
						}
						...
						if(Object_Type.equalsIgnoreCase("captureCusto"))
						{
							FunctionLibrary.captureCusto(driver, Locator_Type, Locator_Value);
							logger.log(LogStatus.INFO, Description);
						}
						if(Object_Type.equalsIgnoreCase("custoTable"))
						{
							FunctionLibrary.custoTable(driver);
							logger.log(LogStatus.INFO, Description);
						}
